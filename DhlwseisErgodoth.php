...
<body>
    ...
    while ($businessRow = $businessesQueryResult->fetch_array()) {
        ...
        // now we need to find all the employees that work in this business and are not at home or halted
        $employeesQuery = 'select distinct Users_ID 
        ...
        // execute the query
        $employeesQueryResult = $mysqli -> query($employeesQuery);

        // loop through the employees found
        while ($employeeRow = $employeesQueryResult->fetch_array()) {
    ...
    if (isset($_SESSION['loggedIn']) && $_SESSION['loggedIn'] == true && $_SESSION['userType'] === 'employer') {
        require 'ActualDhlwseisErgodoth.php';
    } else {
        require 'ErrorMessage.php';
    }
    ?>
    <section class="call-to-action text-white text-center" style="background: url(&quot;bg-masthead.jpg&quot;) no-repeat center center;margin-top: 850px;background-color: #ffffff;background-size: cover;background-image: url(&quot;assets/img/fade.jpg&quot;);height: 400px;">
    ...
    <script src="assets/js/dhlwseisScript.js"></script>
</body>
