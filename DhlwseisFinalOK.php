...
<!-- Here we handle user input, we need to insert data into the database -->
<?php
        require_once 'databaseLogin.php';
        
        if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['dhlwseisButton']))
        {
            injectDhlwseis($hn, $un, $pw, $db);
        }
        function injectDhlwseis($hn, $un, $pw, $db)
        {
            ...
            $splitByComma = preg_split('/,/', $_POST['dhlwseisData']);

            // now we filter the empty ones
            $splitByComma = array_filter($splitByComma, "emptyString");

            // now iterate through them
            foreach($splitByComma as $currEntry){
                // break entry into parts
                $parts = preg_split('/;/', $currEntry);

                // get individual parts
                $homeOrHalt = $parts[0];
                if($homeOrHalt == 'Home'){
                    $tableName = 'remoteperiods';
                } else {
                    $tableName = 'jobhaltings';
                }
                $id = $parts[1];
                $startDate = $parts[2];
                $endDate = $parts[3];
                ...
                // formulate the query
                $insertionQuery = 'insert into ' . $tableName . ' (StartDate, EndDate, Employees_Users_ID) values (DATE \'' . $startDate . '\', DATE \'' . $endDate . '\', ' . $id . ')';
    ...
    <div class="text-center">
        <div class="container-fluid float-left" style="filter: blur(0px) grayscale(0%);">
            <div class="row" style="background-color: rgba(0,0,0,0);margin-top: -360px;margin-bottom: 314px;">
                <div class="col">
                    <h1 style="display:none;">h1</h1><h2 style="display:none;">h2</h2><h3 style="display:none;">h3</h3><h4 style="display:none;">h4</h4><h5 style="color: #ffffff;">Άρση δηλώσεων</h5><a class="btn btn-primary btn-block border rounded border-white" role="button" data-bs-hover-animate="pulse" style="background-color: rgb(23,31,50);color: #ffffff;" href="ArseisDhlwsewn.php"><i class="fa fa-chevron-left" style="margin-right: 10px;"></i>Είσοδος στην εφαρμογή</a></div>
                <div
            ...
            <div class="col">
                ...
                <div class="card" style="height: 400px;background-color: rgba(23,31,50,0.8);width: 70%;margin-left: 15%;">
                    <div class="card-body"><i class="fa fa-check-circle" style="color: #0c7b09;font-size: 80px;"></i>
                        <h1 style="color: #ffffff;background-color: rgba(0,0,0,0);margin: 15px;font-size: 30px;margin-top: 30px;">Οι δηλώσεις που κάνατε αποθηκεύτηκαν επιτυχώς στην βάση δεδομένων μας!<br><br>Έχετε την δυνατότητα να κάνετε άρση μίας ή περισσότερων δηλώσεων αλλά και να δείτε μια γενική κατάσταση των δηλώσεων που έχετε υποβάλει ακολουθώντας
                            τους αντίστοιχους συνδέσμους που φαίνονται στην σελίδα.</h1>
                    </div>
                </div>
            </div>
        </div>
