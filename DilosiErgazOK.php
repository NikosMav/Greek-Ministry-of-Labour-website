...
<body>
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['newEmployeeButton']))
    {
        registerEmployee($hn, $un, $pw, $db);
    } 

    function registerEmployee($hn, $un, $pw, $db){
        // connect to database
        ...
        // we must search for specified details in the users table so we find the employee's ID
        $userQuery = "select ID from `users` where FirstName = \"{$_POST['newEmployeeFirstName']}\" and LastName = \"{$_POST['newEmployeeLastName']}\" and PhoneNumber = \"{$_POST['newEmployeePhoneNumber']}\" and Email = \"{$_POST['newEmployeeEmail']}\"";

        // get the ID corresponding to this query
        $userID = $mysqli->query($userQuery)->fetch_array()['ID'];

        // we now must update this employee to work for this business and employer
        $businessName = $_POST['newEmployeeBusinessName'];
        $employerID = $_SESSION['ID'];

        $updateQuery = "update `employees` set Businesses_BusinessName = \"{$businessName}\", Businesses_Employers_Users_ID = {$employerID} where Users_ID = {$userID}";
    ...
    <section class="call-to-action text-white text-center" style="background: url(&quot;bg-masthead.jpg&quot;) no-repeat center center;margin-top: 458px;background-color: #ffffff;background-size: cover;background-image: url(&quot;assets/img/fade.jpg&quot;);height: 400px;">
    ...
    <div class="footer-dark" style="height: 130px;">
        <footer>
            <div class="container" style="margin-top: -30px;">
                <div class="row">
                    <div class="col item social"><img alt="" src="assets/img/EllhnikhDhmokratiaLogo.png" style="height: 60px;margin-left: 0px;filter: brightness(153%) contrast(117%);margin-right: 4px;"><img alt="" src="assets/img/Flag_of_Europe.png" style="width: 60px;margin-right: 0px;margin-left: 0px;"><img alt="" src="assets/img/Flag_of_Greece.png"
                            style="width: 60px;margin-right: 0px;"><a style="color:#135987" href="https://www.facebook.com/labourgovgr"><i class="icon ion-social-facebook" aria-hidden="true"><span class="sr-only">an icon</span></i></a><a style="color:#135987" href="https://twitter.com/labourgovgr?lang=el"><i class="icon ion-social-twitter" aria-hidden="true"><span class="sr-only">an icon</span></i></a></div>
                </div>
