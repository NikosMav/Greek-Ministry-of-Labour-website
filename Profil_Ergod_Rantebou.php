...
if($appointmentResults->num_rows !== 0){
    ...
    while($appointmentRow = $appointmentResults->fetch_array()){
        ...
        <div class=\"col\" style=\"margin-top: 20px;\">
            <form style=\"margin-left: 50px;margin-right: 30px;\">
                <div class=\"form-group\"><label for=\"commentAria\" style=\"color: rgba(255,255,255,0.9);font-size: 20px;\"><strong>Σχόλια</strong><br></label><textarea id=\"commentAria\" class=\"border rounded border-warning shadow-lg form-control\" style=\"height: 90px;\" readonly=\"\">{$appointmentRow['Comment']}</textarea></div>
            </form>
        </div>
    </div>";
