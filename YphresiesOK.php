...
<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['appointmentButton']))
    {
        newAppointment($hn, $un, $pw, $db);
    } 

    function newAppointment($hn, $un, $pw, $db){
        // see if the user is logged in, only then insert into database
        if (isset($_SESSION['loggedIn']) && $_SESSION['loggedIn'] == true) {
            ...
            // get data for the appointment
            $appointmentDate = $_POST['appointmentDate'];
            $appointmentComment = $_POST['appointmentComment'];
            $userID = $_SESSION['ID'];

            // make the appointmentQuery
            $appointmentQuery = "insert into `Appointments` (Date, Users_ID, Comment) values (DATE \"{$appointmentDate}\", {$userID}, \"{$appointmentComment}\")";
        
            // run the query and be done with it
            $mysqli->query($appointmentQuery);
        }
