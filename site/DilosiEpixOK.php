...
<body>
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['newBusinessButton']))
    {
        registerBusiness($hn, $un, $pw, $db);
    } 

    function registerBusiness($hn, $un, $pw, $db){
        // connect to database
        ...
        // we must make a new bussiness under currently logged in employer with given name (ignore other data for now)
        $employerID = $_SESSION['ID'];

        // insert to business table
        $newBusinessQuery = 'insert into `businesses` (BusinessName, Employers_Users_ID) values ("' . $_POST['businessName'] . '", ' . $employerID . ')';

        $mysqli->query($newBusinessQuery);
    }   
    ?>
